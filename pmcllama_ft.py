#!/usr/bin/env python3
# QLoRA fine-tune of PMC-LLaMA 13B. Very tight on VRAM — bsz=1, grad_accum=32.
...
OUT_DIR = Path("/workspace/LLM_research/treatRag_MIMIC_IV/output_pmcllama_ft")
OUT_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_STATE = 42
TRAIN_RATIO = 0.8
VAL_RATIO = 0.1            # 70/10/20

MODEL = "axiong/PMC_LLaMA_13B"
MAX_SEQ_LEN = 2048
MAX_PROMPT_TOKENS = 1800
MAX_NEW_TOKENS = 128
BATCH_SIZE = 2               # inference: 13B, keep batch small

LORA_R = 16
LORA_ALPHA = 32
LORA_DROPOUT = 0.1
FT_EPOCHS = 5                # was 3; aligned with eICU
FT_BATCH_SIZE = 1            # 13B — bsz=1 is what fits
FT_GRAD_ACCUM = 32           # effective batch 32
FT_LR = 2e-4
FT_WARMUP_RATIO = 0.1
...
# prompt matches the GRU feature set (demographics + full lists + time gaps + visit idx)
...
class MedPredDataset(Dataset):
    """Causal LM fine-tuning dataset (prompt masked in labels)."""
...
def run_inference(prompts: List[str], tokenizer, model, device: str,
    ...
    all_outputs = []
    model.eval()
    for start_idx in tqdm(range(0, len(prompts), batch_size), desc="PMC-LLaMA-FT inference"):
        ...
        with torch.no_grad():
            output_ids = model.generate(
                **encoded,
                ...
                do_sample=False,
                num_beams=4,                    # was greedy in earlier runs; matches eICU now
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("STEP 4: LOADING PMC-LLaMA + LoRA")
    logger.info("=" * 70)
    ...
    logger.info("=" * 70)
    logger.info("STEP 5: FINE-TUNING PMC-LLaMA WITH LoRA")
    logger.info("=" * 70)
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PHASE A: BASELINE Fine-tuned PMC-LLaMA (No RAG)")
    logger.info("=" * 70)
    ...
    baseline_metrics = evaluate_predictions(
        baseline_results, "Fine-tuned PMC-LLaMA (No RAG)")
