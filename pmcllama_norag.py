#!/usr/bin/env python3
"""PMC-LLaMA 13B baseline (no retrieval). NOTE: 13B is tight on a single A100, batch=4."""
...
OUT_DIR = Path("/workspace/LLM_research/treatRag/output_pmcllama_norag")
OUT_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_STATE = 42
TRAIN_RATIO = 0.8
MAX_PROMPT_TOKENS = 1800       # PMC-LLaMA ctx is only 2048
MAX_NEW_TOKENS = 128
LLM_MODEL = "axiong/PMC_LLaMA_13B"
BATCH_SIZE = 4                 # 13B — small batch



# drug name -> ATC level 3. shared with the other LLM scripts.
DRUG_TO_ATC3: Dict[str, str] = {
    ...
    # L - immunosuppressants (transplant pts)
...
def main():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PHASE A: BASELINE PMC-LLaMA 13B (No RAG)")
    logger.info("=" * 70)
    ...
    baseline_metrics = evaluate_predictions(baseline_results, "Baseline PMC-LLaMA 13B (No RAG)")
