#!/usr/bin/env python3
# Meditron-7B no-RAG baseline. Same drug dict used for biomistral/pmcllama variants.
...
OUT_DIR = Path("/workspace/LLM_research/treatRag/output_meditron_norag")
OUT_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_STATE = 42
TRAIN_RATIO = 0.8
MAX_PROMPT_TOKENS = 3500       # Meditron ctx 4096, reserve room for generation
MAX_NEW_TOKENS = 128
...
# drug name -> ATC level 3. Includes brand names since MIMIC mixes them.
...
def main():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PHASE A: BASELINE Meditron-7B (No RAG)")
    logger.info("=" * 70)
    ...
    baseline_metrics = evaluate_predictions(baseline_results, "Baseline Meditron-7B (No RAG)")
