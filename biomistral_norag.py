#!/usr/bin/env python3
"""Zero-shot BioMistral-7B baseline. No retrieval, drug map from a static dict."""
...
OUT_DIR = Path("/workspace/LLM_research/treatRag/output_biomistral_norag")
OUT_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_STATE = 42
TRAIN_RATIO = 0.8
MAX_PROMPT_TOKENS = 3500       # ctx is 32768 but keep this bounded for VRAM
MAX_NEW_TOKENS = 128
LLM_MODEL = "BioMistral/BioMistral-7B"
BATCH_SIZE = 8                 # 7B fits ~8 per batch at this seq len



# Hand-curated drug name -> ATC level 3 lookup. Brand + generic aliases.
DRUG_TO_ATC3: Dict[str, str] = {
    ...
    # J — anti-infectives (the bulk of ICU rx)
    ...
    # N — CNS (sedation, analgesia, AEDs, etc.)
...
def run_llm_inference(
    ...
    for start_idx in tqdm(range(0, len(prompts), batch_size), desc="BioMistral inference"):
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("LOADING BioMistral-7B MODEL")
    logger.info("=" * 70)
    ...
    # baseline pass
    logger.info("\n" + "=" * 70)
    logger.info("PHASE A: BASELINE BioMistral-7B (No RAG)")
    logger.info("=" * 70)
    ...
    baseline_outputs = run_llm_inference(
    ...
    baseline_metrics = evaluate_predictions(baseline_results, "Baseline BioMistral-7B (No RAG)")
