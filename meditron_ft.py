#!/usr/bin/env python3
"""QLoRA fine-tune for Meditron-7B; inference is no-RAG."""
...
OUT_DIR = Path("/workspace/LLM_research/treatRag_MIMIC_IV/output_meditron_ft")
OUT_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_STATE = 42
TRAIN_RATIO = 0.8
VAL_RATIO = 0.1            # 70/10/20 matching the GRU baseline

MODEL = "epfl-llm/meditron-7b"
MAX_SEQ_LEN = 2048          # LLaMA-2 context
...
LORA_ALPHA = 32
LORA_DROPOUT = 0.1
FT_EPOCHS = 5                # was 3; bumped to align with eICU
FT_BATCH_SIZE = 2
FT_GRAD_ACCUM = 16
FT_LR = 2e-4
FT_WARMUP_RATIO = 0.1
...
# prompt mirrors GRU features (demographics, full diag/rx, time deltas, visit idx)
...
class MedPredDataset(Dataset):
    # causal-LM dataset; prompt tokens get -100 so loss only fires on target
...
class PaddingCollator:
    """Left-pad batches; causal LM needs prefix padding for generate()."""
...
def run_inference(prompts: List[str], tokenizer, model, device: str,
    ...
    all_outputs = []
    model.eval()
    for start_idx in tqdm(range(0, len(prompts), batch_size), desc="Meditron-FT inference"):
        ...
        with torch.no_grad():
            output_ids = model.generate(
                **encoded,
                ...
                do_sample=False,
                num_beams=4,                    # aligned with eICU Meditron FT (was num_beams=2)
...
def main():
    ...
    # split: 70/10/20, mirrors the GRU
    ...
    logger.info("=" * 70)
    logger.info("STEP 4: LOADING Meditron + LoRA")
    logger.info("=" * 70)
    ...
    # LoRA on all attn + ffn projections
    ...
    logger.info("=" * 70)
    logger.info("STEP 5: FINE-TUNING Meditron WITH LoRA")
    logger.info("=" * 70)
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PHASE A: BASELINE Fine-tuned Meditron (No RAG)")
    logger.info("=" * 70)
    ...
    baseline_metrics = evaluate_predictions(
        baseline_results, "Fine-tuned Meditron (No RAG)")
