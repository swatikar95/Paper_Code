#!/usr/bin/env python3
"""LoRA fine-tune BioGPT then run no-RAG inference on the held-out split."""
...
OUT_DIR = Path("/workspace/LLM_research/treatRag/output_biogpt_ft")
OUT_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_STATE = 42
TRAIN_RATIO = 0.8
VAL_RATIO = 0.1            # matches GRU split: 70/10/20

BIOGPT_MODEL = "microsoft/biogpt"
MAX_SEQ_LEN = 1024          # ctx
...
# drug -> ATC3 lookup (shared across LLM scripts)
DRUG_TO_ATC3: Dict[str, str] = {
    ...
    # J — anti-infectives (big chunk for ICU)
...
# match the feature set the GRU sees (demographics, full diag/rx lists, time gaps)
def build_enhanced_prompt(visits: pd.DataFrame, include_last_rx: bool = False,
    ...
    # demographics live on the first visit row
    first = visits.iloc[0]
...
class MedPredDataset(Dataset):
    """Causal-LM dataset: prompt + comma-joined ATC-3 target, prompt tokens masked in labels."""
    ...
    def __init__(self, examples: List[Dict], tokenizer, max_length: int = MAX_SEQ_LEN):
        ...
        for ex in tqdm(examples, desc="Tokenizing training data"):
            ...
            # mask prompt tokens so loss only fires on the target
...
class PaddingCollator:
    """Left-pad to longest in batch (required for causal LM generation)."""
...
def run_inference(prompts: List[str], tokenizer, model, device: str,
    ...
    all_outputs = []
    model.eval()
    for start_idx in tqdm(range(0, len(prompts), batch_size), desc="BioGPT-FT inference"):
...
def main():
    ...
    # 70/10/20 split, same as GRU baseline
    ...
    logger.info("=" * 70)
    logger.info("STEP 4: LOADING BioGPT + LoRA")
    logger.info("=" * 70)
    ...
    # target_modules covers BioGPT's attn + ffn projections
    ...
    logger.info("=" * 70)
    logger.info("STEP 5: FINE-TUNING BioGPT WITH LoRA")
    logger.info("=" * 70)
    ...
    logger.info(f"Saved LoRA adapter to {OUT_DIR / 'lora_adapter'}")
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PHASE A: BASELINE Fine-tuned BioGPT (No RAG)")
    logger.info("=" * 70)
    ...
    baseline_metrics = evaluate_predictions(
        baseline_results, "Fine-tuned BioGPT (No RAG)")
