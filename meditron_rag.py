#!/usr/bin/env python3
# Meditron-7B with BioLORD RAG. Chroma collection lives next to OUT_DIR.
...
OUT_DIR = Path("/workspace/LLM_research/treatRag/output_meditron_rag")
...
RANDOM_STATE = 42
TRAIN_RATIO = 0.8
MAX_PROMPT_TOKENS = 3500       # Meditron ctx 4096 — leave gen headroom
MAX_NEW_TOKENS = 128
...
CHROMA_COLLECTION_NAME = "treatrag_meditron_biolord"
CHROMA_DISTANCE_METRIC = "cosine"

# RAG knobs (kept consistent with biogpt/biomistral runs for comparison)
RAG_TOP_K = 30
RAG_MAX_CASES = 10
RAG_MIN_SIMILARITY = 0.3
RAG_THRESHOLD_MULT = 1.5

# drug name -> ATC3 (brand + generic)
...
def run_llm_inference(
    ...
    for start_idx in tqdm(range(0, len(prompts), batch_size), desc="Meditron inference"):
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("LOADING Meditron-7B MODEL")
    logger.info("=" * 70)
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PHASE B: TreatRAG Meditron-7B (With BioLORD-2023 Embedding RAG)")
    logger.info("=" * 70)
    ...
    rag_metrics = evaluate_predictions(rag_results, "TreatRAG Meditron-7B (BioLORD-2023 RAG)")
