#!/usr/bin/env python3
"""BioMistral-7B + BioLORD retrieval over chroma. Logs per-stage timings."""
...
OUT_DIR = Path("/workspace/LLM_research/treatRag/output_biomistral_rag")
...
RANDOM_STATE = 42
TRAIN_RATIO = 0.8
MAX_PROMPT_TOKENS = 3500       # ctx is 32768 but keep memory in check
MAX_NEW_TOKENS = 128
...
CHROMA_COLLECTION_NAME = "treatrag_biomistral_biolord"
...
def main():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PHASE B: TreatRAG BioMistral-7B (With BioLORD-2023 Embedding RAG)")
    logger.info("=" * 70)
    ...
    rag_metrics = evaluate_predictions(rag_results, "TreatRAG BioMistral-7B (BioLORD-2023 RAG)")
